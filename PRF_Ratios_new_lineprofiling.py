...
def getSFSratios(fn,dofolded,isfolded = False):
    ...
    if args.thetaNest_for_thetaNspace:
        thetaNspace = thetaNest
    else:
...
def run(args):
    ...
    # p =[2.3487536291966467, 2.9752587909463415, 3.8853314448574787]
    # p=[2.3487536291966467, 80, 3.8853314448574787]
    # arglis(p,nc,dofolded,includemisspec,densityof2Ns,fix_theta_ratio,max2Ns,usepm,usepm0,fix_mode_0,thetaNspace,zvals)
    # temp = [p] + list(arglist)
    # likelihood = -PRF_Ratios_functions_new.NegL_SFSRATIO_estimate_thetaratio(*temp)
    # confidence_intervals = generate_confidence_intervals(func,list(p), arglist, likelihood,boundsarray)
    ...
    for ii in range(args.optimizetries):
        ...
        # result = OptimizeResult(x=[2.1862,3.9806,0.43651,0.38062,-1.7632],fun=-2.283,message = "debugging")
...
def parsecommandline():
    ...
    if JH_turn_off_options_for_release == False:
        ...
        parser.add_argument("-j",dest ="thetaNest_for_thetaNspace",default = False,action="store_true", help = "use thetaNest in place of thetaNspace, preempts -a ")      
    else:
    ...
    if JH_turn_off_options_for_release == True: # add the things not included in args by parser.parse_args() when this is set 
        ...
        args.thetaNest_for_thetaNspace = False
        args.profile == False
    else:
...
if __name__ == '__main__':
    ...
    if args.profile:
        ...
        import PRF_Ratios_functions_new
        profiler.add_function(PRF_Ratios_functions_new.NegL_SFSRATIO_estimate_thetaratio)
        profiler.add_function(PRF_Ratios_functions_new.intdeltalogprobratio)
        profiler.add_function(PRF_Ratios_functions_new.prfdensityfunction)
        profiler.add_function(PRF_Ratios_functions_new.integrate2Ns)
        profiler.add_function(PRF_Ratios_functions_new.prf_selection_weight)
        profiler.add_function(PRF_Ratios_functions_new.getXrange)
    ...
    if args.profile:
        ...
        # Write the line profiler stats
        stats_file = f'line_profiler_stats_{args.poplabel}.txt'
        with open(stats_file, 'w') as f:
            profiler.print_stats(stream=f)
        print(f"Line profiler stats written to {stats_file}")